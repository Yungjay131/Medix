    ...
    private fun handleBnvMenuItemClick(item:MenuItem):Boolean {
        ...
        return when (item.itemId) {
            R.id.action_home -> {
                _inflateFragment(
                    if (viewModel.getUserAccountType() == DOCTOR)
                        HomeDoctorFragment.newInstance()
                    else
                        HomePatienrFragment.getInstance() )
