        ...
        private fun generateRandomID(limit: Int): String {
            ...
            for (index in 0..limit) {
                when (selctionArray_range.random()) {
                    0 -> {
                        ID = ID.plus(numberArray.get(numberArray_range.random()))
                    }
                    1 -> {
                        ID = ID.plus(lowercaseLettersArray.get(lowercaseArray_range.random()))
                    }
                    2 -> {
                        ID = ID.plus(uppercaseLettersArray.get(uppercaseArray_range.random()))
                    }
                }
            }
