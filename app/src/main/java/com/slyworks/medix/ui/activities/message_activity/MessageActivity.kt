...
class MessageActivity : BaseActivity() {
    ...
    @Inject
    lateinit var mViewModel:MessageActivityViewModel
    //endregion
    ...
    private fun initData(){
        ...
        mViewModel = ViewModelProvider(this).get(MessageActivityViewModel::class.java)
    ...
    private fun initViews2(){
        ...
        fabSend.setOnClickListener {
            val message: Message = Message(
                ...
                senderFullName = mViewModel.getUserDetailsUtils().fullName,
                receiverFullName = mUserProfile.fullName,
                content = etMessage.text.toString().trim(),
                timeStamp = System.currentTimeMillis().toString(),
                messageID = IDUtils.generateNewMessageID(),
                status = NOT_SENT,
                senderImageUri = mViewModel.getUserDetailsUtils().imageUri,
                accountType = mViewModel.getUserDetailsUtils().accountType,
                FCMRegistrationToken = mViewModel.getUserDetailsUtils().FCMRegistrationToken,
