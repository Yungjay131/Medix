...
class VoiceCallActivity : BaseActivity() {
    ...
    @Inject
    lateinit var mViewModel: VoiceCallViewModel
    //endregion
    ...
    private fun initData(){
        ...
        if(type == VOICE_CALL_OUTGOING){
            val request: VoiceCallRequest =
                VoiceCallRequest(mViewModel.getUserDetailsUtils(), REQUEST_PENDING)
                mViewModel.sendVoiceCallRequestViaFCM(mUserDetails)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe {
                    when{
                        it.isSuccess ->{
                            mViewModel.processVoiceCall(TYPE_REQUEST,
    ...
    private fun setViewsForIncomingVoiceCall(details: FBUserDetails){
        mViewModel.vibrate(INCOMING_CALL_NOTIFICATION)
    }
    private fun setViewsForCallStarted(){
        mViewModel.stopVibration()
    }
